...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    /// </summary>
    public class CheckButton : XRBaseInteractable
    {
        class PressInfo
        {
        ...
        //diko mou apo 59 mexri 
        [SerializeField]
        private bool buttonFullyPressed = false;
    ...
    /*
        ...
        [SerializeField]
        private AudioSource audioSource2; // Το AudioSource που θα παίζει τα κλιπ λαθος

        [SerializeField]
        private AudioClip buttonPressClip; // Ήχος όταν πατηθεί το κουμπί σωστο

        [SerializeField]
        private AudioClip buttonPressClip2; // Ήχος όταν πατηθεί το κουμπί λαθος
    ...
    */
        ...
        void UpdatePress()
        {
            ...
            //diko moy
            if (pressed && !buttonFullyPressed)
            {
                buttonFullyPressed = true; // Καταγράφουμε ότι το κουμπί έχει πατηθεί πλήρως
                CheckSockets();
            }

            if (!pressed && buttonFullyPressed)
            {
                buttonFullyPressed = false; // Καταγράφουμε ότι το κουμπί έχει απελευθερωθεί
            }
            //diko mou
        }
        ...
        void OnValidate()
        {
         ...
         // Καλείται όταν θέλετε να ελέγξετε τα sockets (π.χ. με το πάτημα του κουμπιού)
    public void CheckSockets()
    {
        if (AreAllSocketsFilledWithEnergy())
        {
            Debug.Log("Και οι τρεις υποδοχές περιέχουν αντικείμενα με tag 'Energy'.");
            // Εδώ μπορείτε να προσθέσετε ενέργειες που θα γίνουν όταν όλα τα sockets είναι γεμάτα
        }
        else
        {
            Debug.Log("Δεν έχουν συμπληρωθεί όλες οι υποδοχές με το tag 'Energy'.");
        }
    }

    // Ελέγχει αν όλα τα sockets έχουν αντικείμενο με το tag "Energy"
    private bool AreAllSocketsFilledWithEnergy()
    {
        return CheckSocketForEnergy(socket1) && CheckSocketForEnergy(socket2) && CheckSocketForEnergy(socket3);
    }

    // Ελέγχει αν η συγκεκριμένη υποδοχή περιέχει αντικείμενο με το tag "Energy"
    private bool CheckSocketForEnergy(XRSocketInteractor socket)
    {
        if (socket.hasSelection)
        {
            var selectedObject = socket.GetOldestInteractableSelected().transform.gameObject;
            return selectedObject.CompareTag("Energy");
        }
        return false;
    }
